 ...
 *   BUILTIN_N8N_URL — n8n base URL (default: http://localhost:5678)
 *   BUILTIN_N8N_API_KEY — n8n API key for automation
 */
export async function provisionN8nWorkspace(
  orgSlug: string
): Promise<{ url: string; apiKey: string; webhookUrl: string }> {
  const url = process.env.BUILTIN_N8N_URL || "http://localhost:5678";
  const apiKey = process.env.BUILTIN_N8N_API_KEY || "";

  if (!url) {
    throw new Error(
      "BUILTIN_N8N_URL must be set — cannot provision built-in n8n"
    );
  }
